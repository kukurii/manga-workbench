...
class StyleManager {
    ...
    bindEvents() {
        ...
        if (this.btnSavePreset) {
            this.btnSavePreset.addEventListener('click', () => {
                const name = prompt("请为该段落预设起一个好记的名字 (如: 对话-方正宋体-36pt):", "对话样式A");
                if (!name) return;
    ...
    // 给 fontTool.js 回调，用来同步系统字体到此处的下拉菜单
    syncFonts(fontList) {
    ...
    applyStyle(presetObj = null) {
        ...
        if (isNaN(size) || size <= 0) return alert('请输入有效的字号');
        if (isNaN(leadingValue) || leadingValue <= 0) return alert('请输入有效的行距');

        // 发送到 ExtendScript 统一处理段落三参数
        // 为避免空的 fontPostName 变成字面量 'undefined' 传进 JSX 导致报错，必须拦截
